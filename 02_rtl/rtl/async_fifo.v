...
module async_fifo #(
    ...
    // Almost full: 2 slots remaining, based on write-domain occupancy.
    // wr_count over-estimates occupancy while reads lag (rd_ptr sync stale),
    // so gating on it is CONSERVATIVE — prevents overflow even with the
    // 1-cycle registered wr_en and 1-beat/cycle zero-bubble writer.
    // (Gray-plus2 comparison was unsafe: stale rd_ptr sync let a fast
    //  writer overrun the FIFO by 2-3 beats.)
