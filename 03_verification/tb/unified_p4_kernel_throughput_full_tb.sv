...
// Step12F coverage extension: exercise every Gate-A one-dimensional mode
// supported by the unified P4 kernel.  This is deliberately a separate bench
// from the historical seven-mode Gate-B smoke so the original evidence stays
// immutable while the full 13-mode campaign is recorded independently.
module unified_p4_kernel_throughput_full_tb;
    logic clk = 1'b0;
    logic rst_n = 1'b0;
    logic start = 1'b0;
    logic [1:0] tr_type = '0;
    ...
    task automatic run_mode(input integer type_arg, input integer size_arg);
        begin
            ...
            // Start a new vector every group_count accepting cycles.  The
            // four-slot kernel must sustain group II=1 while earlier vectors
            // are being drained; all-zero inputs make the numeric check exact.
            for (cycle_i = 0; cycle_i < (vector_count * group_count + 8);
                ...
                if (out_valid && out_req) begin
                    ...
                    if (out_data !== expected_zero)
                        $fatal(1, "nonzero output type=%0d size=%0d cycle=%0d",
                               type_arg, size_arg, cycle_i);
                end
                #1step;
                if (done)
            ...
            wait_i = 0;
            while ((done_count < vector_count) && (wait_i < 512)) begin
                @(posedge clk);
                if (out_valid && out_req) begin
                    output_count = output_count + 1;
                    if (out_data !== expected_zero)
                        $fatal(1, "nonzero drained output type=%0d size=%0d",
                               type_arg, size_arg);
                end
                #1step;
                if (done)
            ...
            $display("GATE_F_VECTOR_II_PASS type=%0d size=%0d ii=%0d vectors=%0d",
    ...
    initial begin
        ...
        run_mode(2, 16);
        run_mode(2, 32);
        $display("GATE_F_VECTOR_II_TB_PASS modes=13");
        $finish;
    end
endmodule
