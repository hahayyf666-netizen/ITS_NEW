"""Create the V3.5-16 reproducible non-cache SHA-256 manifest."""
from __future__ import annotations
import hashlib, json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "05_audit" / "current" / "16"
MANIFEST = OUT / "V35_16_FREEZE_MANIFEST.json"
...
def main() -> int:
    ...
    for p in ROOT.rglob("*"):
        ...
        if any(part in SKIP for part in p.relative_to(ROOT).parts):
            continue
        if rel.startswith("05_audit/current/16/"):
            continue
        files[rel] = {"bytes": p.stat().st_size, "sha256": sha(p)}
    doc = {"step": "V3.5-16", "root_policy": "workspace-relative",
    ...
    print(f"V3.5-16 manifest: {len(files)} files")
    return 0
