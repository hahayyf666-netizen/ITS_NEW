"""Create a non-cache full-project SHA-256 manifest for Step 12A-R."""
from __future__ import annotations
import hashlib, json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "_step12ar_audit"
MANIFEST = OUT / "STEP12A_R_FREEZE_MANIFEST.json"
...
def main() -> int:
    ...
    for p in ROOT.rglob("*"):
        ...
        if any(part in SKIP for part in p.relative_to(ROOT).parts):
            continue
        if rel.startswith("_step12ar_audit/"):
            continue
        files[rel] = {"bytes": p.stat().st_size, "sha256": sha(p)}
    doc = {"step": "12A-R", "root": str(ROOT), "cache_policy": "excluded",
    ...
    print(f"STEP12A-R manifest: {len(files)} files")
    return 0
