...
module unified_p4_kernel #(
    ...
    // Compile-time transpose only. Runtime selection is one complete P4 bundle.
    ...
    // All bounds/indices in this block are elaboration constants. The source
    // coefficient file, matrix orientation and signed precision are unchanged.
    ...
    always_comb begin
        coeff_bundle_addr = bundle_base(slot_type_q[output_slot_q],
                                       slot_matrix_size_q[output_slot_q]) + output_group_q;
                ...
                for (input_i = 0; input_i < MAX_N; input_i = input_i + 1) begin
                    if (input_i < slot_size_q[output_slot_q]) begin
                        accum_c[lane_i] = accum_c[lane_i] +
                            ($signed(coeff_bundle_c[(lane_i*MAX_N + input_i)*COEFF_W +: COEFF_W]) *
